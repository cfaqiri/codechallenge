...
class UploadFile(APIView):
    ...
    def post(self, request, format=None):
        ...
        check_duplicate_report = PayrollReportService(
        ).check_duplicate_report(name=file.name, user=request.user)
        ...
        PayrollReportService().add_records(
            csv_data=csv_data, name=file.name, user=request.user)
